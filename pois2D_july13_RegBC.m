...
% 1 = Zou/He + LBGK
% 2 = Regularized + LBGK
% 3 = Regularized + RBGK <--- this doesn't seem to work....
% 4 = Regularized + MRT
% 5 = Zou/He + MRT
% 6 = Zou/He + MRT DP
    ...
    % add paths for Jacket libraries
     switch location
        ...
        case 'home'
            ...
            addpath('/home/stu/Dropbox/matlab/jacketSDK/channel2D_RegVwPe_LBGK_ts');
            addpath('/home/stu/Dropbox/matlab/jacketSDK/channel2D_RegVwPe_RBGK_ts');
            addpath('/home/stu/Dropbox/matlab/jacketSDK/channel2D_RegVwPe_MRT_ts');
            addpath('/home/stu/Dropbox/matlab/jacketSDK/channel2D_VW_PE_MRT_ts');
            addpath('/home/stu/Dropbox/matlab/jacketSDK/channel2D_VW_PE_MRT_DPts');
        ...
        case 'office'
            ...
            addpath('/home/srblair/Dropbox/matlab/jacketSDK/channel2D_RegVwPe_LBGK_ts');
            addpath('/home/srblair/Dropbox/matlab/jacketSDK/channel2D_RegVwPe_RBGK_ts');
            addpath('/home/srblair/Dropbox/matlab/jacketSDK/channel2D_RegVwPe_MRT_ts');
            addpath('/home/srblair/Dropbox/matlab/jacketSDK/channel2D_VW_PE_MRT_ts');
            addpath('/home/srblair/Dropbox/matlab/jacketSDK/channel2D_VW_PE_MRT_DPts');
        case 'hamming'
     ...
     for ts = 1:Num_ts
         ...
         switch Boundary_Dynamics
             
             case 1
                 
                 if(mod(ts,2)==1)
                     %fIn->fOut
                     channel2D_VW_PE_LBGK_ts(fOut,fIn,inl_d,onl_d,snl_d,ux_p,rho_out,nu_lbm,int32(Nx),int32(Ny));
                     %rho_dp = sum(fOut(v_data_LP),2);
                     %u_data(ts)=double(((fOut(v_data_LP,:)*ex')/rho_dp)./u_conv_fact);
                 else
                     %fOut->fIn
                     channel2D_VW_PE_LBGK_ts(fIn,fOut,inl_d,onl_d,snl_d,ux_p,rho_out,nu_lbm,int32(Nx),int32(Ny));
             ...
             case 2
                  if(mod(ts,2)==1)
                     %fIn->fOut
                     channel2D_RegVwPe_LBGK_ts(fOut,fIn,inl_d,onl_d,snl_d,ux_p,rho_out,nu_lbm,int32(Nx),int32(Ny));
                     %rho_dp = sum(fOut(v_data_LP),2);
                     %u_data(ts)=double(((fOut(v_data_LP,:)*ex')/rho_dp)./u_conv_fact);
                 else
                     %fOut->fIn
                     channel2D_RegVwPe_LBGK_ts(fIn,fOut,inl_d,onl_d,snl_d,ux_p,rho_out,nu_lbm,int32(Nx),int32(Ny));
                     %rho_dp = sum(fIn(v_data_LP),2);
                    % u_data(ts)=double(((fIn(v_data_LP,:)*ex'))/rho_dp./u_conv_fact);
                  end
             ...
             case 3
                  ...
                  if(mod(ts,2)==1)
                     %fIn->fOut
                     channel2D_RegVwPe_RBGK_ts(fOut,fIn,inl_d,onl_d,snl_d,ux_p,rho_out,nu_lbm,int32(Nx),int32(Ny));
                     %rho_dp = sum(fOut(v_data_LP),2);
                     %u_data(ts)=double(((fOut(v_data_LP,:)*ex')/rho_dp)./u_conv_fact);
                 else
                     %fOut->fIn
                     channel2D_RegVwPe_RBGK_ts(fIn,fOut,inl_d,onl_d,snl_d,ux_p,rho_out,nu_lbm,int32(Nx),int32(Ny));
                    % rho_dp = sum(fIn(v_data_LP),2);
                     %u_data(ts)=double(((fIn(v_data_LP,:)*ex'))/rho_dp./u_conv_fact);
                  end
             ...
             case 4
                 if(mod(ts,2)==1)
                     %fIn->fOut
                     channel2D_RegVwPe_MRT_ts(fOut,fIn,inl_d,onl_d,snl_d,ux_p,rho_out,omega_op,int32(Nx),int32(Ny));
                     %rho_dp = sum(fOut(v_data_LP),2);
                     %u_data(ts)=double(((fOut(v_data_LP,:)*ex')/rho_dp)./u_conv_fact);
                 else
                     %fOut->fIn
                     channel2D_RegVwPe_MRT_ts(fIn,fOut,inl_d,onl_d,snl_d,ux_p,rho_out,omega_op,int32(Nx),int32(Ny));
                     % rho_dp = sum(fIn(v_data_LP),2);
                     % u_data(ts)=double(((fIn(v_data_LP,:)*ex'))/rho_dp./u_conv_fact);
                 end
             ...
             case 5
                 if(mod(ts,2)==1)
                     %fIn->fOut
                     channel2D_VW_PE_MRT_ts(fOut,fIn,inl_d,onl_d,snl_d,ux_p,rho_out,omega_op,int32(Nx),int32(Ny));
                     %rho_dp = sum(fOut(v_data_LP),2);
                     %u_data(ts)=double(((fOut(v_data_LP,:)*ex')/rho_dp)./u_conv_fact);
                 else
                     %fOut->fIn
                     channel2D_VW_PE_MRT_ts(fIn,fOut,inl_d,onl_d,snl_d,ux_p,rho_out,omega_op,int32(Nx),int32(Ny));
                     % rho_dp = sum(fIn(v_data_LP),2);
                     % u_data(ts)=double(((fIn(v_data_LP,:)*ex'))/rho_dp./u_conv_fact);
                 end
             ...
             case 6
                  if(mod(ts,2)==1)
                     %fIn->fOut
                     channel2D_VW_PE_MRT_DPts(fOut,fIn,inl_d,onl_d,snl_d,ux_p,rho_out,omega_op,int32(Nx),int32(Ny));
                     %rho_dp = sum(fOut(v_data_LP),2);
                     %u_data(ts)=double(((fOut(v_data_LP,:)*ex')/rho_dp)./u_conv_fact);
                 else
                     %fOut->fIn
                     channel2D_VW_PE_MRT_DPts(fIn,fOut,inl_d,onl_d,snl_d,ux_p,rho_out,omega_op,int32(Nx),int32(Ny));
                     % rho_dp = sum(fIn(v_data_LP),2);
                     % u_data(ts)=double(((fIn(v_data_LP,:)*ex'))/rho_dp./u_conv_fact);
                 end
