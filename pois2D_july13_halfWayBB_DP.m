...
ux_theory_g = Umax*(1-((gcoord(:,2)-by)/by).^2);
% don't count inlet, outlet or solid nodes in computing the error
gnl = 1:nnodes;
gnl = setxor(gnl,intersect(snl,gnl));
gnl = setxor(gnl,intersect(inl,gnl));
gnl = setxor(gnl,intersect(onl,gnl));
    ...
    % add paths for Jacket libraries
     switch location
        ...
        case 'home'
            ...
            addpath('/home/stu/Dropbox/matlab/jacketSDK/channel2D_VW_PE_LBGK_DPts');
        ...
        case 'office'
            ...
            addpath('/home/srblair/Dropbox/matlab/jacketSDK/channel2D_VW_PE_LBGK_DPts');
     ...
     for ts = 1:Num_ts
         ...
         switch dynamics
             ...
             case 1
                 if(mod(ts,2)==1)
                     %fIn->fOut
                     channel2D_VW_PE_LBGK_DPts(fOut,fIn,inl_d,onl_d,snl_d,ux_p,rho_out,nu_lbm,int32(Nx),int32(Ny));
                     %rho_dp = sum(fOut(v_data_LP),2);
                     %u_data(ts)=double(((fOut(v_data_LP,:)*ex')/rho_dp)./u_conv_fact);
                 else
                     %fOut->fIn
                     channel2D_VW_PE_LBGK_DPts(fIn,fOut,inl_d,onl_d,snl_d,ux_p,rho_out,nu_lbm,int32(Nx),int32(Ny));
     ...
     nnodes_gnl = length(gnl);
     g_err = sum(abs(ux_theory_g(gnl)-ux_h(gnl))/nnodes_gnl);
     fprintf('Global Error Measure = %g.\n',g_err);
